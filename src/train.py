...
from models.Nets import feature_extractor, task_classifier, GeneDistrNet, Discriminator
...
def args_parser():
    ...
    paser.add_argument('--weight-dec', type=float, default=1e-5, help='weight decay coefficient default 1e-5')
    paser.add_argument('--rp-size', type=int, default=1024, help='Random Projection size')
    paser.add_argument('--epochs', type=int, default=10, help='rounds of local training')
    paser.add_argument('--factor', type=float, default=0.2, help='lr decreased factor (0.1)')
    paser.add_argument('--patience', type=int, default=20, help='number of epochs required before reduce lr (20)')
    ...
    paser.add_argument('--input_size', type=int, default=2048, help='the input size of distribution generator')
    paser.add_argument('--hidden_size', type=int, default=4096, help='the size of hidden feature')  # 4096-alex
    paser.add_argument('--num_labels', type=int, default=5, help='the number of label class')  # vlcs-5
    paser.add_argument('--global_epochs', type=int, default=20, help='the number of iterative training')
    ...
    for iteration in range(4*local_runs):
        ...
        for t in range(args.global_epochs):
            ...
            for i in range(3):
                ...
                acc, w, wd = trainer.train()  # local_accuracy, parameters of global model, local discriminator
                w_locals.append(w)
                local_d[i] = wd
                avg_ac += acc
                torch.cuda.empty_cache()
